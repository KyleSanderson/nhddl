...
static void viewOptionsPopupDraw(View *v, int zOrder) {
  ...
  // Header
  fontRenderString(FONT_DEFAULT, centerX, POPUP_Y + SCENE_KEEPOUT, FONT_ALIGN_HCENTER, 0, 0, zOrder, "View options", HeaderTextColor);
  // Entries
  int entryY = POPUP_Y + SCENE_KEEPOUT + SCENE_ROW_SIZE;
  for (int i = 0; i < ENTRY_COUNT; i++) {
    ...
    fontRenderInRect(FONT_DEFAULT, leftX, entryY, rightX, entryY + SCENE_ROW_SIZE, FONT_ALIGN_LEFT | FONT_ALIGN_VCENTER, zOrder + 1, entries[i],
                     col);
...
static void deviceFilterPopupDraw(View *v, int zOrder) {
  ...
  uiDrawRectRounded(gsGlobal, vx1, vy1, vx2, vy2, POPUP_ROUNDRECT_RADIUS, zOrder, ModalBGColor);
  // Header
  fontRenderString(FONT_DEFAULT, centerX, POPUP_Y + SCENE_KEEPOUT, FONT_ALIGN_HCENTER, 0, 0, zOrder, "Filter devices", HeaderTextColor);
  // Devices
  int entryY = POPUP_Y + SCENE_KEEPOUT + SCENE_ROW_SIZE;
  ...
  for (int i = 0; i < (n + 1); i++) {
    ...
    fontRenderInRect(FONT_DEFAULT, leftX + INDICATOR_R * 4, entryY, rightX, entryY + SCENE_ROW_SIZE, FONT_ALIGN_LEFT | FONT_ALIGN_VCENTER,
                     zOrder + 1, label, col);
    entryY += SCENE_ROW_SIZE;
  }

  uiDrawPrompt(gsGlobal, POPUP_PROMPT_SLOT_CENTER_X(vx1, vx2, 0), POPUP_PROMPT_ROW_Y(vy2), POPUP_PROMPT_ROW_Y(vy2), zOrder, HeaderTextColor,
               ICON_CROSS, "Toggle");
  uiDrawPrompt(gsGlobal, POPUP_PROMPT_SLOT_CENTER_X(vx1, vx2, 1), POPUP_PROMPT_ROW_Y(vy2), POPUP_PROMPT_ROW_Y(vy2), zOrder, HeaderTextColor,
               ICON_CIRCLE, "Back");
}
